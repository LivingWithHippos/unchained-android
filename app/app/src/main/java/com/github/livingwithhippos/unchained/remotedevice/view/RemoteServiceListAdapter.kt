...
class RemoteServiceListAdapter(listener: ServiceListListener): DataBindingTrackedAdapter<RemoteService, ServiceListListener>(DiffCallback(), listener) {
    class DiffCallback : DiffUtil.ItemCallback<RemoteService>() {
        ...
        override fun areContentsTheSame(oldItem: RemoteService, newItem: RemoteService): Boolean {
            return oldItem.isDefault == newItem.isDefault && oldItem.name == newItem.name && oldItem.port == newItem.port
        }
    }
...
class ServiceKeyProvider(private val adapter: RemoteServiceListAdapter) : ItemKeyProvider<RemoteService>(SCOPE_MAPPED) {
    override fun getKey(position: Int): RemoteService? =
